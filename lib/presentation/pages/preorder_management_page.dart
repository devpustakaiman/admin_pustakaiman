...
class _PreorderManagementPageState extends State<PreorderManagementPage> {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // 1. Settings Card for Notification Email
                _buildEmailSettingsCard(controller),
                const SizedBox(height: 20),

                // 2. Settings Card for Pre-Order Bank Accounts
                _buildBankAccountsSettingsCard(context, controller),
                const SizedBox(height: 28),

                // 3. Pre-Order Orders Section & Filters
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'Kelola pesanan pre-order buku dan konfigurasi alamat email notifikasi penerima.',
                    style: TextStyle(
  ...
  // ---------------- 1. SETTINGS CARD FOR NOTIFICATION EMAIL ----------------
  Widget _buildEmailSettingsCard(PreorderController controller) {
    return Container(
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.mail, color: Color(0xFF0F766E), size: 18),
              ),
              const SizedBox(width: 14),
              Expanded(
                child: Column(
                  ...
                  children: const [
                    Text(
                      'Pengaturan Email Notifikasi Pre-Order',
                      style: TextStyle(
                    ...
                    SizedBox(height: 2),
                    Text(
                      'Alamat email di bawah ini akan menerima notifikasi setiap ada rincian pemesanan pre-order baru.',
                      style: TextStyle(
                        color: Color(0xFF64748B),
                        fontSize: 12,
                      ),
            ...
            builder: (context, constraints) {
              ...
              final inputWidget = TextField(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: const Icon(LucideIcons.mail, color: Color(0xFF64748B), size: 18),
                  filled: true,
              ...
              final saveButton = Obx(() {
                final isSaving = controller.isSavingEmail.value;
                return ElevatedButton.icon(
                  onPressed: isSaving ? null : controller.saveNotificationEmail,
                  icon: isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
  ...
  // ---------------- 2. SETTINGS CARD FOR PRE-ORDER BANK ACCOUNTS ----------------
  Color _getBankBadgeColor(String bankName) {
    final lower = bankName.toLowerCase();
    if (lower.contains('bca')) return const Color(0xFF00569B);
    if (lower.contains('mandiri')) return const Color(0xFF003D79);
    if (lower.contains('bni')) return const Color(0xFFF15A24);
    if (lower.contains('bri')) return const Color(0xFF00529C);
    if (lower.contains('bsi')) return const Color(0xFF00A39D);
    if (lower.contains('cimb')) return const Color(0xFFED1C24);
    if (lower.contains('permata')) return const Color(0xFF7CB342);
    if (lower.contains('seabank')) return const Color(0xFFFF5722);
    return const Color(0xFF0F766E);
  }

  Widget _buildBankAccountsSettingsCard(BuildContext context, PreorderController controller) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.white,
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(LucideIcons.landmark, color: Color(0xFF0F766E), size: 18),
                    ),
                    const SizedBox(width: 14),
                    Expanded(
                      child: Column(
                        ...
                        children: const [
                          ...
                          SizedBox(height: 2),
                          Text(
                            'Daftar rekening bank aktif penerima pembayaran transfer untuk transaksi pre-order.',
                            style: TextStyle(
                              color: Color(0xFF64748B),
                              fontSize: 12,
                            ),
          ...
          Obx(() {
            if (controller.bankAccounts.isEmpty) {
              return Container(
                ...
                child: Column(
                  children: [
                    const Icon(LucideIcons.creditCard, size: 36, color: Color(0xFF94A3B8)),
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: _getBankBadgeColor(account.bankName),
                              ...
                              Row(
                                children: [
                                  IconButton(
                                    ...
                                    icon: const Icon(LucideIcons.edit3, size: 16, color: Color(0xFF0F766E)),
  ...
  void _confirmDeleteBank(BuildContext context, PreorderController controller, BankAccountModel account) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 10),
            Text('Hapus Rekening Bank', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ],
        ),
        content: Text(
          'Apakah Anda yakin ingin menghapus rekening bank ${account.bankName} (${account.accountNumber})?',
  ...
  // ---------------- 3. TABLE CONTROLS (SEARCH & WRAP FILTERS) ----------------
